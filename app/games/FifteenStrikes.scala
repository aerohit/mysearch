...
case class FifteenStrikes(player1: Player, player2: Player, var sticks: Int = 15) {
  ...
  def strike(player: Player, howMany: Int): Unit = {
    if (howMany < 1 || howMany > 3) throw InvalidRequestException("Must strike either 1, 2 or 3")
    lastPlayer.map { lp =>
      if (lp.id == player.id) throw InvalidRequestException("A player cant move consecutively")
    }
